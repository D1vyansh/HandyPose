...
class WASPv2(nn.Module):
    def __init__(self, backbone, inplanes, planes, n_classes=21):
        ...
        self.conv2_lower = nn.Conv2d(512, 48, 1, bias=False)
        self.conv2_lowest = nn.Conv2d(1024, 48, 1, bias=False)
    ...
    def forward(self, x, low_level_features, lower_level_features, lowest_level_features):
        input = x
        x1 = self.aspp1(x)
        x2 = self.aspp2(x1)
        x3 = self.aspp3(x2)
        ...
        lower_level_features = self.conv2_lower(lower_level_features)
        lower_level_features = self.bn2(lower_level_features)
        lower_level_features = self.relu(lower_level_features)
        
        lowest_level_features = self.conv2_lowest(lowest_level_features)
        lowest_level_features = self.bn2(lowest_level_features)
        lowest_level_features = self.relu(lowest_level_features)
        
        low_level_feat = self.maxpool(low_level_features)
        x = F.interpolate(x, size=low_level_feat.size()[2:], mode='bilinear', align_corners=True)
        low_level_features = F.interpolate(low_level_features, size=low_level_feat.size()[2:], mode='bilinear', align_corners=True)
        lower_level_features = F.interpolate(lower_level_features, size=low_level_feat.size()[2:], mode='bilinear', align_corners=True)
        lowest_level_features = F.interpolate(lowest_level_features, size=low_level_feat.size()[2:], mode='bilinear', align_corners=True)
