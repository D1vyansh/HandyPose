...
from utils       import cmu_data             as cmu_data
from utils       import mpii_nzsldata        as mpii_nzsldata
from utils       import veri_data            as veri_data
...
def draw_paint(im, kpts, mapNumber, epoch, model_arch, dataset):
    ...
    im = cv2.resize(cv2.imread(im),(368,368))
    # limbSeq for CMU Hand Dataset
    limbSeq = [[0,1], [1,2], [2,3], [3,4], [0,5], [5,6], [6,7], [7,8], [0,9], [9,10], [10,11], [11,12], [0,13], [13,14], 
        [14,15], [15,16], [0,17], [17,18], [18,19], [19,20]]
    # limbSeq for VeRi Dataset
    #limbSeq = [[0, 1], [0, 5], [0, 10], [7, 13], [5, 7], [7, 8], [8,6], [4, 6], [6, 12], [2, 4], [2, 11], [2, 3], [13, 12], 
    #                [12, 15], [13, 14], [14, 15], [14, 16], [16, 1], [15, 17], [17, 3], [16,17], [4,9], [9,5]]
    # draw points
    for k in kpts:
        x = k[0]
        y = k[1]
        if x != 0 and y != 0:
            cv2.circle(im, (x, y), radius=4, thickness=-1, color=(0, 0, 255))
    ...
    for i in range(len(limbSeq)):
        ...
        if X0!=0 and Y0!=0 and X1!=0 and Y1!=0:
            if i < 4:
                cv2.line(cur_im, (Y0,X0), (Y1,X1), [255,0,255], 1)
            elif i >= 4 and i < 8:
                cv2.line(cur_im, (Y0,X0), (Y1,X1), [255,255,000], 1)
            elif i >= 8 and i < 12:
                cv2.line(cur_im, (Y0,X0), (Y1,X1), [000,255,000], 1)
            elif i >=12 and i < 16:
                cv2.line(cur_im, (Y0,X0), (Y1,X1), [255,000,000], 1)
            else:
                cv2.line(cur_im, (Y0,X0), (Y1,X1), [0,0,255], 1)

        im = cv2.addWeighted(im, 0.2, cur_im, 0.8, 0)

    cv2.imwrite('samples/WASPpose/'+str(mapNumber)+'.png', im)
...
def getDataloader(dataset, train_dir, val_dir, test_dir, sigma, stride, workers, batch_size):
    if dataset == 'LSP':
        ...
        train_loader = torch.utils.data.DataLoader(
                                            cmu_data.cmu(train_dir, sigma, "train",
                                            Mytransforms.Compose([Mytransforms.TestResized(368),])),
                                            batch_size  = batch_size, shuffle=True,
                                            num_workers = workers, pin_memory=True)
    
        val_loader   = torch.utils.data.DataLoader(
                                            cmu_data.cmu (val_dir, sigma, "valid",
                                            Mytransforms.Compose([Mytransforms.TestResized(368),])),
    ...
    elif dataset == "MPII_NZSLHand":
        train_loader = torch.utils.data.DataLoader(
                                            mpii_nzsldata.mpii_nzsl(train_dir, sigma, "train",
                                            Mytransforms.Compose([Mytransforms.TestResized(368),])),
                                            batch_size  = batch_size, shuffle=True,
                                            num_workers = workers, pin_memory=True)
    
        val_loader   = torch.utils.data.DataLoader(
                                            mpii_nzsldata.mpii_nzsl (val_dir, sigma, "valid",
                                            Mytransforms.Compose([Mytransforms.TestResized(368),])),
                                            batch_size  = 1, shuffle=False,
                                            num_workers = 1, pin_memory=True)
        ...
        train_loader = torch.utils.data.DataLoader(
                                            veri_data.VeRi(train_dir, sigma, "train",
                                            Mytransforms.Compose([Mytransforms.TestResized(368),])),
                                            batch_size  = batch_size, shuffle=True,
                                            num_workers = workers, pin_memory=True)
    
        val_loader   = torch.utils.data.DataLoader(
                                            veri_data.VeRi (val_dir, sigma, "valid",
                                            Mytransforms.Compose([Mytransforms.TestResized(368),])),
                                            batch_size  = 1, shuffle=True,
                                            num_workers = 1, pin_memory=True)
